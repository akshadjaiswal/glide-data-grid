...
const sparklineRenderer: CustomRenderer<SparklineCell> = {
  ...
  draw: (args, cell) => {
    ...
    // Light area fill for readability
...
const personaRenderer: CustomRenderer<PersonaCell> = {
  ...
  draw: (args, cell) => {
    ...
    const image = imageLoader.loadOrGetImage(cell.data.avatar, col, row)
    if (!image) {
...
const gridThemeOverrides: Partial<Theme> = {
  ...
  headerFontStyle: '600 13px "Inter", -apple-system, BlinkMacSystemFont, sans-serif',
  headerIconSize: 18,
  baseFontStyle: '400 14px "Inter", -apple-system, BlinkMacSystemFont, sans-serif',
  markerFontStyle: '600 12px "Inter", -apple-system, BlinkMacSystemFont, sans-serif',
...
export function EmployeeGrid({ rows }: EmployeeGridProps) {
  const theme = useMemo(() => ({ ...getDefaultTheme(), ...gridThemeOverrides }), [])
  ...
  const columns = useMemo<readonly GridColumn[]>(() => {
    ...
    return [
      ...
      { id: 'photo', title: 'Photo', group: 'Info', width: 96, icon: icon.HeaderImage },
      ...
      switch (columnId) {
        ...
        case 'photo':
          return {
            ...
            data: [rowData.photo],
            displayData: [`${rowData.firstName} ${rowData.lastName}`],
            allowOverlay: true,
            rounding: 8,
            readonly: true,
          }
        ...
        case 'hiredAt':
          return {
            kind: GridCellKind.Text,
            data: format(rowData.hiredAt, 'EEE MMM dd yyyy'),
            displayData: format(rowData.hiredAt, 'EEE MMM dd yyyy'),
            allowOverlay: false,
          }
        default:
          return {
  ...
  const getCellsForSelection = useCallback(
    (selection: Rectangle, _abortSignal?: AbortSignal) => {
